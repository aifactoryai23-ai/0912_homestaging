...
export default function TransformPage() {
  ...
  // 🔧 Авто-сборка builderPrompt из правой колонки через promptEngine
  useEffect(() => {
    const next = buildBuilderPrompt({
      activeTab,
      iRoom,
      iStyle,
      iColors,
  ...
  const handleGenerate = async () => {
    console.log(
      "%c>>> handleGenerate CALLED",
      "background: #ff0000; color: white; padding: 2px 6px;",
      performance.now(),
    );

    const finalPrompt = buildFinalPrompt({ customPrompt, builderPrompt });
    ...
    // 🔐 серверная проверка и списание кредита через RPC
    try {
      console.log(
        "%c>>> CALLING RPC consume_credit_if_active NOW",
        "background: #ff8800; color: black; padding: 2px 6px;",
        performance.now(),
      );
  ...
  const handlePresetClick = (presetKey) => {
    const promptValue = buildPresetPrompt({
      presetKey,
      presetMapping,
      activeTab,
      iRoom,
    ...
    // Мягкий вариант: пресет добавляется к уже написанному тексту
  ...
  return (
    <>
      <Helmet>
        <title>AI Virtual Staging — Transform Photos | HomeDesign Pro</title>
        <meta
          name="description"
          content="Upload a real estate photo and instantly generate professionally staged interiors with AI. Perfect for listings, Airbnb and renovation planning."
        />
        <link rel="canonical" href="https://homedesignpro.ai/transform" />
      </Helmet>
